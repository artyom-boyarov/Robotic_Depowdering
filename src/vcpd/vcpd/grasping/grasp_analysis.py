...
def main():
    ...
    for obj_name in obj_list:
        ...
        obj_center_of_mass = np.array(obj_mesh.center_mass)
        print(obj_name, " has center of mass at ", obj_center_of_mass)
                ...
                for j in range(num_intersects):
                    if min_score[j] >= cfg['th_min']:
                        ...
                        for angle_idx in range(cfg['num_angle']):
                            ...
                            can_place_fingers, f1pos, f1rot, f2pos, f2rot = rg.place_fingers(widths[j], centers[j], rot_mat[0:3, 0], rot_mat[0:3, 1], rot_mat[0:3, 2], quat[angle_idx])
                            ...
                            if (not rg.is_collided([])) and can_place_fingers:
                                ...
                                grasp_info['min_sing_val_G'].append(grasp_eval.min_singular_value_G(obj_center_of_mass, np.array(f1rot), np.array(f1pos), np.array(f2rot), np.array(f2pos)))
                                grasp_info['vol_ellipsoid_in_wrench'].append(grasp_eval.vol_ellipsoid_in_wrench(obj_center_of_mass, np.array(f1rot), np.array(f1pos), np.array(f2rot), np.array(f2pos)))
                                print(grasp_info['min_sing_val_G'][-1])
        ...
        print("Minimal singular value grasp")
        max_min_sing_val_idx = np.argmax(grasp_info['min_sing_val_G'])
        
        rg.place_fingers(
            grasp_info['widths'][max_min_sing_val_idx],
            grasp_info['centers'][max_min_sing_val_idx], 
            grasp_info['x_directions'][max_min_sing_val_idx], 
            grasp_info['y_directions'][max_min_sing_val_idx], 
            grasp_info['z_directions'][max_min_sing_val_idx], 
            grasp_info['quaternions'][max_min_sing_val_idx]
        )
        x = input()

        print("Maximal volume of ellipsoid in wrench space grasp")
        max_vol_ellipsoid_wrench_idx = np.argmax(grasp_info['vol_ellipsoid_in_wrench'])
        rg.place_fingers(
            grasp_info['widths'][max_vol_ellipsoid_wrench_idx],
            grasp_info['centers'][max_vol_ellipsoid_wrench_idx], 
            grasp_info['x_directions'][max_vol_ellipsoid_wrench_idx], 
            grasp_info['y_directions'][max_vol_ellipsoid_wrench_idx], 
            grasp_info['z_directions'][max_vol_ellipsoid_wrench_idx], 
            grasp_info['quaternions'][max_vol_ellipsoid_wrench_idx]
        )
        x = input()
